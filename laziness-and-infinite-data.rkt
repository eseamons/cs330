...
(define nums (all-integers-from 0))

(define (build-infinite-list f)
  (map f nums))

(define primes (build-infinite-list prime?))
;--------------------------------------------------------------------------------------------------
...
(define (prime?/fast n)
  (cond
    [(< n 2) #f]
    [(= n 2) #t]
    [else (andmap (lambda (x) (not (= (remainder n x) 0)))
                  (take-while (lambda (x) (<= x (floor (sqrt n))))
                              primes/fast))]))


(define primes/fast (filter prime?/fast nums))
